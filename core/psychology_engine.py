...
class PsychologyEngine:
    ...
    def update_curiosity(self, topic: str, exploration_depth: str = "medium"):
        ...
        """
        try:
            with open(self.drives_file, encoding="utf-8") as f:
                drives = json.load(f)
    ...
    def record_expression_need(
        self, expression_type: str, content: str, intensity: int = 5
    ):
        """
        记录表达欲驱动的创作或倾诉
    ...
    def record_emotion_event(
        self, trigger_event: str, initial_feeling: str, intensity: int = 5
    ):
        """
        记录触发情绪的事件和初始感受
        ...
        try:
            ...
            logger.info(
                f"[心理引擎] 情绪事件已记录: {trigger_event} ({initial_feeling})"
            )

        except Exception as e:
            logger.error(f"[心理引擎] 记录情绪事件失败: {e}")

    def update_emotion_phase(
        self, emotion_id: str, phase: str, note: str | None = None
    ):
        """
        更新情绪生命周期的当前阶段
    ...
    def add_moral_principle(self, principle: str, context: str | None = None):
        """
        添加道德原则
