...
class LifeStoryEngine:
    ...
    def _load_life_story(self) -> dict:
        ...
        if self.life_story_file.exists():
            try:
                with open(self.life_story_file, encoding="utf-8") as f:
                    return json.load(f)
            except Exception as e:
    ...
    async def collect_recent_experiences(self, days: int = None) -> dict:
        ...
        data = {"conversations": [], "thoughts": [], "events": [], "growth_changes": []}
        ...
        try:
            ...
            if self.personality_evolution:
                ...
                if self_awareness:
                    # 检查最近的特质变化
                    traits = self_awareness.self_description.get(
                        "personality_traits", []
                    )
            ...
            logger.info(
                f"[人生故事引擎] 收集到 {len(data['conversations'])} 条对话, "
    ...
    def _build_story_update_prompt(self, recent_data: dict) -> str:
        ...
        if recent_data.get("conversations"):
            ...
            for conv in recent_data["conversations"][:10]:  # 最多10条
                prompt_parts.append(
                    f"- {conv['time'][:10]}: 与{conv['user']}讨论了{conv['topic']}"
                )
        ...
        if recent_data.get("growth_changes"):
            ...
            if growth.get("current_interests"):
                prompt_parts.append(
                    f"当前兴趣：{', '.join(growth['current_interests'])}"
                )
    ...
    def _integrate_story_update(self, story_update: str, recent_data: dict):
        """整合故事更新到人生故事中"""
        try:
            # 添加新章节（注意：current_chapter 已在 update_life_story 中递增）
            chapter = {
        ...
        try:
            ...
            if response and hasattr(response, "completion_text") and response.completion_text:
                ...
                self.context_cache = {
                    ...
                    "cache_valid_until": datetime.now().timestamp()
                    + (86400 * self.cache_days),  # 使用配置的缓存天数
                ...
                logger.info(
                    f"[人生故事引擎] 精简上下文已生成，约{self.context_cache['estimated_tokens']}token"
                )
    ...
    def get_context_for_llm(self) -> str:
        ...
        if current_time < cache_valid:
            ...
            if compact:
                logger.debug(
                    f"[人生故事引擎] 使用缓存的上下文，约{len(compact) // 2}token"
                )
                return compact

        # Cache expired, return base persona
        base_persona = self.state.get("base_persona", "")
        if base_persona:
            logger.debug("[人生故事引擎] 缓存失效，返回基础人设")
        return base_persona

    def get_story_summary(self) -> str:
        """获取人生故事摘要（供外部查看）"""
        total_chapters = len(self.life_story.get("timeline", []))
        total_events = len(self.life_story.get("key_experiences", []))
        return (
            f"人生故事引擎状态：\n"
            f"- 当前章节：{self.state.get('current_chapter', 0)}\n"
