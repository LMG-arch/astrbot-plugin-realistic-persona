...
class MemoryManager:
    ...
    def __init__(self, data_dir: Path):
        ...
        # 对话记录文件（带权重）
        self.weighted_conversations_file = (
            self.data_dir / "weighted_conversations.jsonl"
        )
        # 记忆强化档案（频繁回顾的记忆）
    ...
    def _init_data_files(self):
        ...
        if not self.memory_reinforcement_file.exists():
            self.memory_reinforcement_file.write_text(
                json.dumps(
                    {
    ...
    def calculate_memory_importance(
        self, conversation: dict[str, Any], context_clues: list[str] | None = None
    ) -> float:
        """
        计算对话的重要性评分 (0-1)
        ...
        emotional_count = sum(
            1 for indicator in emotional_indicators if indicator in full_text
        )
    ...
    def record_weighted_conversation(
        self,
        user_id: str,
        user_message: str,
        bot_response: str,
        ...
        try:
            ...
            for conv in conversations:
                ...
                if age_days > days_threshold and importance < 0.4:
                    # 低重要性 + 时间久 = 应该淘汰
                    decay_factor = max(
                        0, 1 - (age_days - days_threshold) / 90
                    )  # 90天完全遗忘
        ...
        try:
            ...
            for conv in conversations:
                if conv["timestamp"] == memory_id:
                    ...
                    conv["importance_score"] = max(
                        0, conv.get("importance_score", 0.5) - 0.3
                    )
                    break
    ...
    def get_important_memories(
        self, user_id: str | None = None, threshold: float = 0.7, limit: int = 10
    ) -> list[dict[str, Any]]:
        """
        检索高重要性记忆
    ...
    def get_memory_statistics(self) -> dict[str, Any]:
        ...
        try:
            ...
            important = sum(
                1 for c in conversations if c.get("importance_score", 0) >= 0.7
            )
            trivial = sum(1 for c in conversations if c.get("is_trivial", False))
            average_importance = sum(
                c.get("importance_score", 0) for c in conversations
            ) / max(total, 1)
    ...
    def get_memory_reinforcement_summary(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "core_categories": self._analyze_core_memory_categories(
                    data.get("core_memories", [])
                ),
            }
    ...
    def _analyze_core_memory_categories(
