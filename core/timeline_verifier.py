...
class TimelineVerifier:
    ...
    def _init_data_files(self):
        ...
        if not self.experience_graph_file.exists():
            self.experience_graph_file.write_text(
                json.dumps(
                    {
                        "nodes": {},  # 经历节点
                        "edges": [],  # 经历之间的关联
    ...
    def add_experience(
        ...
        try:
            ...
            if not consistency_check["valid"]:
                logger.warning(
                    f"[时间线] 时间一致性检查失败: {consistency_check['reason']}"
                )
            ...
            timeline_data["sequences"].append(
                {"id": experience_id, "date": normalized_date, "type": event_type}
            )
            ...
            self._update_experience_graph(
                experience, timeline_data.get("experiences", {})
            )
    ...
    def _normalize_date(self, date_input: str) -> str | None:
        """
        规范化日期格式
    ...
    def _check_consistency(
        self, new_experience: dict[str, Any], timeline_data: dict[str, Any]
    ) -> dict[str, Any]:
        """
        检查新经历的时间一致性
            ...
            if related_id in timeline_data.get("experiences", {}):
                ...
                if "之后" in new_experience.get("content", ""):
                    if new_date < related_date:
                        conflicts.append(
                            f"时间逻辑错误：声称在'{related_id}'之后，但日期更早"
                        )
                ...
                if "之前" in new_experience.get("content", ""):
                    if new_date > related_date:
                        conflicts.append(
                            f"时间逻辑错误：声称在'{related_id}'之前，但日期更晚"
                        )
    ...
    def _log_conflict(
        self, experience: dict[str, Any], conflict: dict[str, Any]
    ) -> None:
        """记录时间线冲突"""
        try:
            conflict_record = {
    ...
    def _update_experience_graph(
        self, experience: dict[str, Any], all_experiences: dict[str, Any]
    ) -> None:
        """更新经历关联图"""
        try:
            with open(self.experience_graph_file, encoding="utf-8") as f:
                ...
                if related_id in all_experiences:
                    edge = {
                        ...
                        "relationship": self._infer_relationship(
                            experience, all_experiences.get(related_id, {})
                        ),
                    }
    ...
    def _infer_relationship(
        self, new_exp: dict[str, Any], related_exp: dict[str, Any]
    ) -> str:
        """
        推断两个经历之间的关系类型
    ...
    def analyze_experience_coherence(
        self, user_experiences: list[dict[str, Any]]
    ) -> dict[str, Any]:
        """
        分析用户经历的整体连贯性
        ...
        try:
            ...
            relationship_density = (
                len(edges) / max(nodes_count - 1, 1) if nodes_count > 1 else 0
            )
            ...
            coherence_score = self._calculate_coherence_score(
                time_span, dict(type_distribution), relationship_density, len(conflicts)
            )
