...
class PostOperator:
    ...
    async def _pipeline(
        ...
        if int(target_id) in self.config.get("ignore_users", []):  # 忽略用户
            ...
            if isinstance(data, dict):
                if code := data.get("code"):
                    if code in [-10031]:
                        ...
                        ignore_users.append(int(target_id))
                        self.config["ignore_users"] = ignore_users
                        logger.warning(
    ...
    async def publish_feed(
        ...
        if llm_text and not text:
            ...
            text = await self.llm.generate_diary(
                persona_profile=persona_profile, user_id=user_id
            )
            ...
            if diary_for_image:
                try:
                    ...
                    if (
                        ...
                        logger.info(
                            f"[配图] event为None或无group_id，使用user_id: {user_id}"
                        )

                    # 传入group_id和user_id让大模型获取对话历史
                    img_prompt = await self.llm.generate_image_prompt_from_diary(
                        diary_for_image, group_id=group_id, user_id=user_id
                    )
                    if img_prompt:
                        img_url = await self.llm._request_image_with_fallback(
                            img_prompt
                        )
                        images = [img_url]
        ...
        try:
            ...
            for post in data:
                ...
                if str(post.uin) != bot_uin:
                    logger.debug(
                        f"[自动回复] 跳过别人的说说: {post.tid}, 作者uin={post.uin}"
                    )
                    continue
                ...
                for comment in detail.comments:
                    ...
                    if str(comment.uin) == bot_uin:
                        logger.debug(
                            f"[自动回复] 跳过自己的评论: {comment.content[:20]}..."
                        )
                        continue  # 不回复自己
