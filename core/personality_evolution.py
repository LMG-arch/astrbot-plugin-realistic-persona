        ...
        if self.state_file.exists():
            try:
                with open(self.state_file, encoding="utf-8") as f:
                    data = json.load(f)
                    self.self_description = data.get(
                        "self_description", self.self_description
                    )
                    self.behavior_stats = data.get(
                        "behavior_stats", self.behavior_stats
                    )
                    # 恢复 defaultdict
                    self.behavior_stats["trait_manifestations"] = defaultdict(
                        int, self.behavior_stats.get("trait_manifestations", {})
                    )
        ...
        if self.state_file.exists():
            try:
                with open(self.state_file, encoding="utf-8") as f:
                    ...
                    self.favorite_phrases = data.get(
...
class HabitBalanceSystem:
    ...
    def __init__(self, data_dir: Path):
        ...
        # 临时习惯（会演化）
        self.temporary_habits = ["最近喜欢说'有意思'", "偶尔会用网络热词"]
        ...
        if self.state_file.exists():
            try:
                with open(self.state_file, encoding="utf-8") as f:
                    ...
                    self.temporary_habits = data.get(
                    ...
                    if last_date_str:
                        self.last_change_date = datetime.fromisoformat(
                            last_date_str
                        ).date()

                    self.last_surprise_time = data.get("last_surprise_time", 0)
                    self.surprise_count_24h = data.get("surprise_count_24h", 0)
                    self.surprise_reset_time = data.get(
                        "surprise_reset_time", time.time()
                    )
    ...
    def should_trigger_surprise(self) -> bool:
        ...
        # 概率递增（距离上次越久，概率越高）
        hours_since_last = (
            (now - self.last_surprise_time) / 3600 if self.last_surprise_time else 24
        )
