...
- `enable_auto_nickname`: 是否自动修改昵称（默认false）
- `enable_auto_signature`: 是否自动修改签名（默认true）
- `enable_auto_avatar`: 是否自动生成并更换头像（默认false）
- `profile_update_cooldown`: 更新冷却时间（默认1800秒，即30分钟）
- `emotion_change_threshold`: 情绪变化阈值（默认0.6，范围0-1）
